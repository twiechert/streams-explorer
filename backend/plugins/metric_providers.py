...
class BurrowPrometheusMetric(Enum):
    ...
    MESSAGES_IN = (
        "messages_in",
        "sum by(topic) (rate(kafka_burrow_partition_current_offset[5m]))",
    )
    MESSAGES_OUT = (
        "messages_out",
        'sum by(topic) (rate(kafka_burrow_current_partition_lag{status="OK", group=~".+"}[5m]) >= 0)',
    )
    CONSUMER_LAG = (
        "consumer_lag",
        'sum by(group) (kafka_burrow_current_partition_lag{status="OK", group=~".+"})',
    )
    CONSUMER_READ_RATE = (
        "consumer_read_rate",
        'sum by(group) (rate(kafka_burrow_current_partition_lag{status="OK", group=~".+"}[5m]) >= 0)',
    )
    TOPIC_SIZE = (
        "topic_size",
...
class BurrowPrometheusMetricProvider(MetricProvider):
    def __init__(self, nodes: List[Tuple[str, dict]]):
        super().__init__(nodes)
        self._prom = PrometheusConnect(url=settings.prometheus.url)

    def get_metric(self, metric: BurrowPrometheusMetric) -> List:
        try:
    ...
    def __get_messages_in(self) -> Dict[str, float]:
        prom_messages_in = self.get_metric(metric=BurrowPrometheusMetric.MESSAGES_IN)
        return {
    ...
    def __get_messages_out(self) -> Dict[str, float]:
        prom_messages_out = self.get_metric(metric=BurrowPrometheusMetric.MESSAGES_OUT)
        return {
            d["metric"]["topic"]: round(float(d["value"][-1]), 2)
    ...
    def __get_consumer_lag(self) -> Dict[str, int]:
        prom_consumer_lag = self.get_metric(metric=BurrowPrometheusMetric.CONSUMER_LAG)
    ...
    def __get_consumer_read_rate(self) -> Dict[str, float]:
        prom_consumer_read_rate = self.get_metric(
            metric=BurrowPrometheusMetric.CONSUMER_READ_RATE
        )
        return {
    ...
    def __get_topic_size(self) -> Dict[str, int]:
        prom_topic_size = self.get_metric(metric=BurrowPrometheusMetric.TOPIC_SIZE)
        return {d["metric"]["topic"]: int(d["value"][-1]) for d in prom_topic_size}

    def __get_replicas(self) -> Dict[str, int]:
        prom_replicas = self.get_metric(metric=BurrowPrometheusMetric.REPLICAS)
        return {d["metric"]["deployment"]: int(d["value"][-1]) for d in prom_replicas}

    def __get_connector_tasks(self) -> Dict[str, int]:
        prom_connector_tasks = self.get_metric(metric=BurrowPrometheusMetric.CONNECTOR_TASKS)
        return {
